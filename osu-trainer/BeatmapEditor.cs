...
namespace osu_trainer
{
    ...
    public class BeatmapEditor
    {
        ...
        public string makeOszOfFolder(string exportDirectory, string songFolder)
        {
            string outputOszName = Path.GetFileNameWithoutExtension(songFolder) + ".osz";
            string outputOsz = Path.Combine(exportDirectory, outputOszName);
        ...
        public string makeOszOfBeatmap(string exportDirectory, Beatmap beatmap, bool includeBg, bool includeMp3)
        {
            string songFolder = new FileInfo(beatmap.Filename).Directory.FullName;
            string outputOszName = Path.GetFileNameWithoutExtension(songFolder) + ".osz";
            string outputOsz = Path.Combine(exportDirectory, outputOszName);
            if (File.Exists(outputOsz))
                File.Delete(outputOsz);
            ...
            try
            {
                using (var fileStream = new FileStream(outputOsz, FileMode.CreateNew))
                {
                    using (ZipArchive archive = new ZipArchive(fileStream, ZipArchiveMode.Create))
                    {
                        archive.CreateEntryFromFile(beatmap.Filename, Path.GetFileName(beatmap.Filename));

                        if (includeMp3)
                        {
                            string audioFilename = Path.Combine(songFolder, beatmap.AudioFilename);
                            archive.CreateEntryFromFile(audioFilename, Path.GetFileName(audioFilename));
                        }
                        if (includeBg)
                        {
